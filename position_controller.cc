  ...
  class PositionPlugin : public ModelPlugin
  {
    ...
    public: virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
      ...
      if (_sdf->HasElement("jointname"))
        ...
        std::cerr << "We find the joint 1111111 [" <<
        joint_name << "]\n";
      ...
      this->model = _model;
      std::cerr << "\n The model's name is 11111[" <<
        _model->GetScopedName() << "]\n";

      //std::cerr<<"******"<<_model->GetJoint() 
 
      
      
      // Setup a P-controller, with a gain of 0.1.
      this->pid = common::PID(p_gain,
       i_gain, d_gain);
      
      //JointPtr joint_ptr=this->_model->GetJoint(joint_name)
 
     // Set the joint's target position.
      this->model->GetJointController()->SetJointPosition(joint_name, position);
      ...
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<std_msgs::Float32>(
            "/" + this->model->GetName() + "/" + joint_name_ori + "/joint_position",
            1,
    ...
    public: void SetPosition(const double &_pos)
    {
      ...
      this->model->GetJointController()->SetJointPosition(joint_name, _pos);
    }
